    ...
    if boundary in QK_NORM_BOUNDARIES:
        ...
        record.update({
            ...
            "input_metrics": compute_metrics(
                norm_actual,
                norm_expected,
                mask.unsqueeze(-1).expand(norm_actual.shape[:-1]) if mask is not None and mask.shape != norm_actual.shape[:-1] else mask,
            ),
        })
        a = actual.float()
        e = expected.float()
        diff = (a - e).abs()
        # Boolean indexing over [batch, tokens] preserves the head axis:
        # [valid_tokens, heads, head_dim].
        if mask is not None:
            diff = diff[mask]
        ...
        location = record["max_error_location"]
        location["head"] = int(location["feature"] // actual.shape[-1])
        location["head_dim"] = int(location["feature"] % actual.shape[-1])
    record["layer"] = layer
    return record
